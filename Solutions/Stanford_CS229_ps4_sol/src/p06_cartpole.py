...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    expected_value = mdp_data['value'] @ mdp_data['transition_probs'][state]
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    trans_counts = mdp_data['transition_counts']
    num_counts = trans_counts.sum(axis=1)
    num_states = trans_counts.shape[0]
    for i in range(num_states):
        for a in range(2):
            if  num_counts[i, a]  != 0:
                mdp_data['transition_probs'][i, :, a] = trans_counts[i, :, a] / num_counts[i, a]

    reward_counts = mdp_data['reward_counts']
    for k in range(num_states):
        sum_counts = reward_counts[k, 1]
        if sum_counts != 0:
            mdp_data['reward'][k] = -reward_counts[k, 0] / sum_counts
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        cur_value = mdp_data['value']
        new_value = mdp_data['reward'] + gamma * (cur_value @ trans_prob).max(axis=1)
        mdp_data['value'] = new_value

        if np.max(np.abs(new_value - cur_value)) < tolerance:
            break
