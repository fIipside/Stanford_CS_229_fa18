...
def backward_softmax(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    # grad = 0 iff cross-entropy = 0
    # return forward_softmax(x) - (grad_outputs != 0).astype(int)
    # Generally
    s = forward_softmax(x)
    return (np.diag(s) - np.outer(s, s)) @ grad_outputs
    # *** END CODE HERE ***
    ...
    for x in range(input_width - conv_width + 1):
        for y in range(input_height - conv_height + 1):
            for output_channel in range(conv_channels):
                ...
                grad_x[:, x:(x + conv_width), y:(y + conv_height)] += output_grad[output_channel, x, y] * conv_W[output_channel, :, :, :]
    ...
    for i in range(input_channels):
        for x in range(0, input_width, pool_width):
            for y in range(0, input_height, pool_height):
                ...
                window = data[i, x:(x + pool_width), y:(y + pool_height)]
                grads[i, x:(x + pool_width), y:(y + pool_height)][i, np.unravel_index(window.argmax(), window.shape)] = output_grad[i, x // pool_width, y // pool_height]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    grad_w = np.outer(data, output_grad)
    grad_b = output_grad
    grad_x = weights @ output_grad
    return (grad_w, grad_b, grad_x)
    # *** END CODE HERE ***
